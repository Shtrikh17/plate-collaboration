...
export function Cursor({
  ...
  const { style, name, selectionStyle = style } = data ?? ({} as CursorData);
  return (
    <>
      {!disableSelection &&
      ...
      {!disableCaret && caretPosition && (
        ...
        {!disableCaret && caretPosition && (
                <div
                    ...
                    style={{ left: caretPosition.left, top: caretPosition.top - 20, backgroundColor: style?.backgroundColor, color: style?.color }}
                >
                    {name}
                </div>
        )}
